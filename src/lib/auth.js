...
export const auth = betterAuth({
	...
	emailAndPassword: {
		...
		sendResetPassword: async ({ user, url, token }, request) => {
			try {
				...
				if (!result.success) {
					console.warn(
						`Falha ao enviar e-mail de alteração de senha para ${user.email}`,
					);
				} else {
					console.log(
						`E-mail de alteração de senha enviado para ${user.email}`,
					);
				}
	...
	emailVerification: {
		...
		sendVerificationEmail: async ({ user, url, token }) => {
			try {
				...
				if (!result.success) {
					console.warn(
						`Falha ao enviar e-mail de verificação para ${user.email}`,
					);
				} else {
	...
	advanced: {
		...
		cookies: {
			session_token: {
				name: isDev ? "session_token" : "__Secure-session_token",
				attributes: {
					httpOnly: true,
					sameSite: "lax",
					path: "/",
					secure: !isDev,
					domain: isDev ? undefined : ".projectdice.com.br",
				},
			},
		},
	},
